...
# --- Configuration ---
DB_FILE = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "config", "dx_patch_library.sqlite3"))
DEFAULT_PAGE_SIZE = 100
RATING_CHOICES = ["Any", "Unrated"] + list(range(1, 11))
COLUMNS_TO_FILTER_TEXT = ["patchname", "category", "bankfile", "comments", "origin"]
# --- Column Config: order & visibility ---

COLUMN_CONFIG = [
    {"id": "id",         "label": "ID",       "width": 40},
    {"id": "patchname",  "label": "Patch",    "width": 90},
...
def setup_tab():
    ...
    def fetch_data(filter_text, filter_rating, page_num, page_size):
        ...
        try:
            ...
            base_query = f"SELECT {', '.join(COLUMNS_TO_DISPLAY)} FROM patches"
            count_query = "SELECT COUNT(*) FROM patches"
            where_clauses = []
            params = []
    ...
    last_selected_patch = gr.State((None, None))
    patch_bank_internal = gr.State([(None, "Init")] * 32)

    # Row 1: Titles
    with gr.Row():
        with gr.Column(scale=6):
            gr.Markdown("# Patch Database Browser")
        with gr.Column(scale=4):
            gr.Markdown("# Construct Patch Bank")
    ...
    with gr.Row():
        ...
        with gr.Column(scale=4):
            gr.Text(value="This bank will be used in the Perform Edit tab.\n"
                          "- Click slot to place current patch\n"
                          "- Randomize populates bank with 32 patches from selection\n"
    ...
    # Row 5: Main DataFrame  |  Patch‑bank DataFrame
    with gr.Row():
        with gr.Column(scale=6):
        ...
        with gr.Column(scale=4):
            patch_bank_display = gr.Dataframe(
                value=[[i + 1, "Init"] for i in range(32)],
                headers=["Slot", "Patch Name"],
                datatype=["number", "str"],
                col_count=(2, "fixed"),
                row_count=(32, "fixed"),
                max_height=750,
    ...
    def update_view(text_filter, rating_filter, page_size, requested_page):
        ...
        status_message = gr.Text(label=f"→ Page: {current_pg} / {total_pgs}", value=f"Found {total_rows} matching patches.", interactive=False)
    ...
    def handle_row_select(evt: gr.SelectData, current_data):
        ...
        try:
            ...
            patch_id = selected_row.iloc[0]
            patch_name = selected_row.iloc[1]
            conn = connect_to_db(DB_FILE)
            _, _, sysex_data = get_patch_from_db(conn, patch_id)
            send_patch_to_buffer(sysex_data, 1, midi_output)
            return f"Selected: ID {patch_id} - {patch_name}", (patch_id, patch_name)
        except Exception as e:
            return f"Status: Error processing selection ({type(e).__name__}).", (None, None)

    def handle_bank_click(evt: gr.SelectData, last_patch, bank_internal):
        if evt.index is None or last_patch[0] is None:
            return gr.update(), bank_internal
        row_idx = evt.index[0]
        patch_id, patch_name = last_patch

        # Update internal list
        bank_internal[row_idx] = (patch_id, patch_name)
        update_patch_bank(row_idx, patch_name)

        # Prepare visible table (slot number + patch names)
    ...
    def handle_randomize_bank(filter_text, filter_rating):
        ...
        try:
            ...
            base_query = f"SELECT id, patchname FROM patches"
            where_clauses = []
            params = []
            ...
            if not all_matches:
                return (
                    gr.skip(),  # Leave patch_bank_display unchanged
                    gr.skip(),  # Leave patch_bank_internal unchanged
                    "No patches available to randomize from."
                )
            ...
            for i, (patch_id, patch_name) in enumerate(bank_entries):
                update_patch_bank(i, patch_name)
            display_rows = [[i + 1, name] for i, (_, name) in enumerate(bank_entries)]

            return (
                gr.update(value=display_rows),
                bank_entries,
                f"Bank randomized: {len(selected)} patch(es), {init_slots} INIT."
            )
    ...
    def handle_send_to_tx802(bank_internal):
        try:
            ...
            patch_ids = [str(pid) for pid, _ in bank_internal if pid is not None]

            if not patch_ids:
                return "⚠️ No valid patches in bank. Nothing sent."

            with tempfile.NamedTemporaryFile(delete=False, suffix=".syx") as temp:
                temp_path = temp.name

            success = create_bank(temp_path, patchfiles=None, db_path=DB_FILE, patchids=",".join(patch_ids))

            if not success:
                os.remove(temp_path)
                return "❌ Failed to create patch bank."

            send_success = send_bank(temp_path, device_id=1, output_port=midi_output)

            # os.remove(temp_path)

            return "✅ Sent patch bank to TX802." if send_success else "❌ Failed to send bank."
    ...
    init_bank_btn.click(handle_init_bank, inputs=[], outputs=[patch_bank_display, patch_bank_internal])
    randomize_bank_btn.click(handle_randomize_bank, inputs=[filter_text_input, filter_rating_input], outputs=[patch_bank_display, patch_bank_internal, status_text])
    send_bank_btn.click(
        handle_send_to_tx802,
        inputs=[patch_bank_internal],
        outputs=[status_text]
    )

    output_dataframe.select(handle_row_select, inputs=[output_dataframe], outputs=[status_text, last_selected_patch])
